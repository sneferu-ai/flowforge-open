...
| GET | `/mock-idp/userinfo` | Bearer token | 200 claims |
...
SOD delivery requirements (preserve the complete product):
Declare installation, data initialization/migrations, startup, readiness and
shutdown. Ship a root Dockerfile and packaging.json for the application, plus
all required UI, documentation and service definitions. Choose languages and
dependencies to meet the specification. Never replace a required database,
worker, external integration or security boundary with a weaker substitute.
The post-specification environment assessment supplies the actual supported
tools, services and connection names before construction. Test substitutes
must remain separate from evidence for real integrations. A setup check is
not product verification. The delivered package must run on a clean machine
using its documented setup and preserve persistent data across restarts.
packaging.json declares runtime_test_command with the literal {port},
runtime_test_mode (http or playwright), health_endpoint (HTTP 200 only when
usable), requirements_file (the actual language's manifest), system_deps and
env_var_names. Declare container_port when startup uses a particular port;
otherwise the container port is 8000. SOD provides that port in PORT, and the
image entrypoint must serve it. Browser features require browser journeys. The Dockerfile's
entrypoint executes the product's actual initialization and startup.
If env_var_names declares APP_BASE_URL, SOD supplies the actual public origin
for launch proof and Preview. Portable launch supplies a localhost default;
the customer's explicit product.env can override it. Keep the application's
same-origin protections and use its configured address instead of a hardcoded
development address. APP_BASE_URL cannot also name a service connection.